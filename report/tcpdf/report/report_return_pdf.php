...
if ($year == 'all' && $dataMonth == 'all') {
	$sqlGetData = 'SELECT * FROM lent_return WHERE status = 1';
	$months = 'ทุกเดือน';
	$year = 'ทุกปีการศึกษา';

} elseif ($year == 'all') {
	$sqlGetData = 'SELECT * FROM lent_return WHERE date_lent LIKE "%-'.$dataMonth.'-%" AND status = 1';
	$year = 'ทุกปีการศึกษา';

} elseif ($dataMonth == 'all') {
	$sqlGetData = 'SELECT * FROM lent_return WHERE year = '.$year.' AND status = 1';
	$months = 'ทุกเดือน';

} else {
	$sqlGetData = 'SELECT * FROM lent_return WHERE date_lent LIKE "%-'.$dataMonth.'-%" AND year LIKE "%'.$year.'%" AND status = 1';
...
while ($fetchArray = mysql_fetch_array($query)) {
	...
	$date_return = $fetchArray['date_return'];
	...
	$dateInputReturn = date('j F Y', strtotime($fetchArray['date_return']));
	$explodeDateReturn = explode(" ", $dateInputReturn);
	$dataYearReturn = $explodeDateReturn[2]+543;

	switch($explodeDateReturn[1]) {
		case "January": $monthReturn = "มกราคม"; break;
		case "February": $monthReturn = "กุมภาพันธ์"; break;
		case "March": $monthReturn = "มีนาคม"; break;
		case "April": $monthReturn = "เมษายน"; break;
		case "May": $monthReturn = "พฤษภาคม"; break;
		case "June": $monthReturn = "มิถุนายน"; break;
		case "July": $monthReturn = "กรกฎาคม"; break;
		case "August": $monthReturn = "สิงหาคม"; break;
		case "September": $monthReturn = "กันยายน"; break;
		case "October": $monthReturn = "ตุลาคม"; break;
		case "November": $monthReturn = "พฤศจิกายน"; break;
		case "December": $monthReturn = "ธันวาคม"; break;
	}

	$dateReturn = $explodeDateReturn[0].' '.$monthReturn.' '.$dataYearReturn;
	...
	if ($i == 1) {
		$html .= '<table align="center"><tr><td><font size="18">รายงานการคืนวัสดุครุภัณฑ์</font></td></tr>
...
$pdf->lastPage();
$pdf->Output('report_return.pdf', 'I');
